...
export default function Footer() {
  ...
  return (
    <footer className="flex flex-col bg-[#263238] mt-10 text-white">
      <div className="flex max-md:flex-col flex-wrap justify-start gap-5 lg:gap-40 sm:px-16 px-8 py-1 lg:py-24">
        <div>
          ...
          {FooterLinks.map((link, index) => (
            <div key={index}>
              {link.links.map((item, index) => (
                <div key={index}  className="mb-3 text-[#C5C5C5]">{item.title}</div>
              ))}
            </div>
          ))}
        </div>
      ...
      <p className="text-center #text-[#C5C5C5] mb-16 lg:mb-5 lg:ml-[460px] mt-4 lg:-mt-8">
